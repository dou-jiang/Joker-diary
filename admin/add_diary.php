...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_diary'])) {
    $title = $_POST["title"];
    ...
    // 插入日记数据到数据库
    $stmt = $conn->prepare("INSERT INTO diaries (title, content, category, publish_time, image_path) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $title, $content, $category, $publish_time, $image_links_json);
...
<body class="bg-gray-100 min-h-screen flex flex-col">
    <div class="flex-1 flex">
        ...
        <aside id="sidebar" class="fixed top-0 left-0 h-full bg-sidebar-default shadow-lg z-50 transition-all duration-300">
            ...
            <nav class="p-4 space-y-1">
                ...
                <a href="add_diary.php" class="sidebar-item active">
        ...
        <main id="main-content" class="flex-1 p-6 main-content main-content-expanded">
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center">
                    ...
                    <h1 class="text-2xl font-bold text-gray-800">添加日记</h1>
                </div>
                ...
                <form method="post" action="" enctype="multipart/form-data">
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="title" id="title" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary transition-custom" placeholder="请输入日记标题">
                        </div>

                        <div>
                            <label for="content" class="block text-sm font-medium text-gray-700 mb-1">内容</label>
                            <textarea name="content" id="content" rows="10" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary transition-custom" placeholder="请输入日记内容"></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="publish_time" id="publish_time" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary transition-custom">
                        </div>
                            ...
                            <div id="image-previews" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4">
                                <?php if (!empty($uploaded_images)): ?>
                                    <?php foreach ($uploaded_images as $index => $image_url): ?>
                                        <div class="image-preview" data-url="<?php echo $image_url; ?>">
                                            <img src="<?php echo $image_url; ?>" alt="预览图" class="w-full h-24 object-cover rounded-md">
                        ...
                        <div class="pt-2">
                            <button type="submit" name="submit_diary" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-custom">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('publish_time').value = `${year}-${month}-${day}T${hours}:${minutes}`;
            ...
            function uploadFiles(files) {
                ...
                xhr.open('POST', 'add_diary.php', true);
