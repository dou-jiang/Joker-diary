...
// 获取庆祝记录信息
$stmt = $conn->prepare("SELECT id, name, qq, diary_id FROM celebrations WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE celebrations SET name = ?, qq = ?, avatar_url = ?, diary_id = ? WHERE id = ?");
    $stmt->bind_param("sssii", $name, $qq, $avatar_url, $diary_id, $id);
        ...
        <main class="flex-1 p-6">
            ...
            <div class="mb-6 flex justify-between items-center">
                <div class="flex items-center">
                    ...
                    <h2 class="text-xl font-bold text-gray-800">编辑庆祝记录</h2>
                </div>
                ...
                <form method="post">
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="<?php echo $celebration['name']; ?>" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary transition-custom" placeholder="请输入姓名">
                        </div>
                        
                        <div>
                            <label for="qq" class="block text-sm font-medium text-gray-700 mb-1">QQ号</label>
                            <input type="text" name="qq" id="qq" value="<?php echo $celebration['qq']; ?>" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary transition-custom" placeholder="请输入QQ号">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="diary_id" id="diary_id" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary transition-custom">
                                <?php foreach ($diaries as $diary): ?>
                                    <option value="<?php echo $diary['id']; ?>" <?php if ($diary['id'] === $celebration['diary_id']) echo 'selected'; ?>><?php echo $diary['title']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
